 ...
 * @Date 6/23/2016
 */
public class KafkaRestPipelineTest {
    ...
    public void ListTopics() throws Exception {
        final String url = "http://localhost:8081/topics";
        final Map<String, String> headers = new HashMap<>();
        headers.put("Accept", "application/vnd.kafka.v1+json, application/vnd.kafka+json, application/json");

        String result = Unirest.get(url).headers(headers).asJson().getBody().toString();
        System.out.println(result);

        // This should be a smarter check
        Boolean testFlag = result.contains("\"_schemas\"");
